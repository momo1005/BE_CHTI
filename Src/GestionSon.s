...
; si on a atteint le max on incremente plus index (on va toujours jouer le meme echantillon - fin du son car plus de vibration différente)
	...
	push {R4} ;met dans la pile l'adresse de l'index
	...
	push {r6} ;on stock dans un registre la valeur de
	ldr R6,=auxmul
	ldr R6,[R6]
	push {r7}
	ldr R7,=auxdiv
	...
	beq SINON ; on saute a sinon si on a fini de parcourir le tableau
...
FINSI
	...
	pop{r7}
	pop{r6}
	pop{r5} ;besoin de remettre la valeur de base
	pop{r4} ; besoin de remettre la valeur de base
